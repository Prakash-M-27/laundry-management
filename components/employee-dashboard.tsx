...
import { ORDER_STATUS_COLORS, OrderStatus, Order } from '@/lib/mock-data'
...
export function EmployeeDashboard({ employeeId }: EmployeeDashboardProps) {
  ...
  const getUpdate = (order: Order): OrderUpdate => {
  ...
  const handleField = (orderId: string, field: keyof OrderUpdate, value: any) => {
    setOrderUpdates(prev => ({
      ...prev,
      [orderId]: { ...getUpdate(orders.find(o => o.id === orderId) || orders[0]), [field]: value },
    }))
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-3">
        {summaryStats.map(stat => (
          <Card key={stat.label} className="p-4 bg-white border-border shadow-sm">
      ...
      <div className="space-y-3">
        {activeOrders.length === 0 ? (
          <Card className="p-10 text-center bg-white border-border shadow-sm">
          ...
          activeOrders.map(order => {
            ...
            return (
              <Card key={order.id} className="bg-white border-border shadow-sm overflow-hidden">
                <button
                    ...
                    <div className="bg-white rounded-xl border border-border shadow-sm p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            placeholder="e.g. 4.5"
                            className="w-full px-3 py-2 rounded-lg border border-border bg-white text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition"
                          />
                        </div>
                        <div>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          rows={2}
                          className="w-full px-3 py-2 rounded-lg border border-border bg-white text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition resize-none"
                        />
                      </div>
